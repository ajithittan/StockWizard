...
const ChartViewer = (props) =>{
    ...
    useEffect(() =>{
      if(props.changes){
        if (props.changes.action === "R"){
          adjustSelections(props.changes.type,props.changes.value)
        }
        else if (props.changes.action === "A" && props.changes.type === "sma"){
          handleChanges(props.changes.type,props.changes.value)
        }
      }
    },[props.changes])
                ...
                stock && width > 0 ? 
                  <>
                    ...
                    <LineChart key={Math.round(width) + stock + charData + initialSetUp?.duration} chartData={charData} fullData={fullData}
                              ...
                              displayfrom={initialSetUp?.duration} addOns={addLines}/>
